<?php
class Article_AdminArticleManagerController extends Zendvn_Controller_Action{
	...
	public function indexAction(){
		...
		$this->view->Items = $tblArticle->listItem($this->_arrParam, array('task'=>'admin-list'));
			
		$totalItem  = $tblArticle->countItem($this->_arrParam);
			...
			if (!empty($_FILES)) {
				...
				//luu vao csdl				
	                ...
	                else
	                {
	                	...
	                	{
	                		$tblArticle=new Article_Model_Article();
		    				$tblArticle->saveItem($this->_arrParam,array('task'=>'admin-add'));			
	...
	public function addAction(){
		$this->view->Title = 'Article :: Article manager :: Add';
		$this->view->headTitle($this->view->Title,true);
		...
		if($this->_request->isPost()){
			...
			if($validate->checkError() == true){
				...
				//Luu vao database
				$rs			= $validate->getData();			
				$tblArticle->saveItem($rs,array('task'=>'admin-add'));
	...
	public function infoAction(){
		$this->view->Title = 'Article :: Article manager :: Information';
		$this->view->headTitle($this->view->Title,true);
		
		$tblArticle = new Article_Model_Article();
		$this->view->Item = $tblArticle->getItem($this->_arrParam,array('task'=>'admin-info'));
		
	}
	
	public function editAction(){
		$this->view->Title = 'Article :: Article manager :: Edit';
		$this->view->headTitle($this->view->Title,true);
		...
		if($this->_request->isPost()){
			...
			if($validate->checkError() == true){
				...
				//Luu vao database
				$rs			= $validate->getData();			
				$tblArticle->saveItem($rs,array('task'=>'admin-edit'));
			}
		}
       
		$this->view->Item = $tblArticle->getItem($this->_arrParam,array('task'=>'admin-edit'));
	...
	public function deleteAction(){
		$this->view->Title = 'Article :: Article manager :: Delete';
		$this->view->headTitle($this->view->Title,true);
		if($this->_request->isPost()){
			$tblArticle = new Article_Model_Article();
			$tblArticle->deleteItem($this->_arrParam,array('task'=>'admin-delete'));
			$this->_redirect($this->_actionMain);
		}
	}
	...
	public function statusAction(){
		$tblArticle = new Article_Model_Article();		
		$tblArticle->changeStatus($this->_arrParam);
		$this->_redirect($this->_actionMain . '/page/'.$this->_request->getParam('page',1));	
		$this->_helper->viewRenderer->setNoRender();
	}
	
	public function isadultAction(){
		$tblArticle = new Article_Model_Article();		
		$tblArticle->changeIsAdult($this->_arrParam);
		$this->_redirect($this->_actionMain . '/page/'.$this->_request->getParam('page',1));	
		$this->_helper->viewRenderer->setNoRender();
	}
	...
	public function ishotAction(){
		$tblArticle = new Article_Model_Article();		
		$tblArticle->changeIsHot($this->_arrParam);
		$this->_redirect($this->_actionMain . '/page/'.$this->_request->getParam('page',1));	
		$this->_helper->viewRenderer->setNoRender();
	}
	...
	public function multiDeleteAction(){
		...
		if($this->_request->isPost()){
			$tblArticle = new Article_Model_Article();
			$tblArticle->deleteItem($this->_arrParam,array('task'=>'admin-multi-delete'));
			$this->_redirect($this->_actionMain);
		}
		$this->_helper->viewRenderer->setNoRender();
	}
	...
	public function sortAction(){
		if($this->_request->isPost()){
			$tblUser = new Article_Model_Article();
	...
	public function getListDataAction()
	{
	        ...
	        $document=Zendvn_Dom_Process::newDocumentHTML($respone);
          	...
          	$objItem=new Article_Model_Article();
