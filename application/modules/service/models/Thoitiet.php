<?php
class Service_Model_Thoitiet extends Zend_Db_Table{
	var $arrTP=array(
      23=>array(
        'khuvuc'=>'Nam Bộ',
        'citys'=>array(
  ...
  {
    $arrTemp=array();
    foreach ($this->arrTP as $k1 => $v1) {
      foreach ($v1['citys'] as $k2 => $v2) {
          $arrTemp[$k2]=$v2;
      }
    ...
    foreach ($this->arrTP as $k1 => $v1) {
      ...
      foreach ($v1['citys'] as $k2 => $v2) {
        $client->setUri('http://www.nchmf.gov.vn/web/vi-VN/62/'.$k1.'/'.$k2.'/map/Default.aspx');
...
*/
    ...
    public function getNewsThoitiet()
    {
        $MaxNews = 5;
        $arrNews = array();
        $client = new Zend_Http_Client();
        $client->setUri('http://www.nchmf.gov.vn/web/vi-VN/69/1/Default.aspx');
        $client->setConfig(array(
          'maxredirects' => 2,
          'timeout'      => 30,   
        ...
        for($i=0;$i<$MaxNews;$i++)
        {
            ...
            $arrNews[$i]['link'] = 'http://www.nchmf.gov.vn/'.trim(pq('.tieude_tintuc:eq('.$i.')')->attr('href'));
            $arrNews[$i]['content'] = trim(pq('.tieude_tintuc:eq('.$i.')')->children('strong')->html());
            //echo $arrNews[$i],'<br>';
        }
        //var_dump($arrNews);
        return $arrNews;
