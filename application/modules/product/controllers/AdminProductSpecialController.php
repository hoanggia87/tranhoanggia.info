<?php
class Product_AdminProductSpecialController extends Zendvn_Controller_Action{
	...
	public function indexAction(){
		...
		$this->view->Title = 'Product :: Product specical manager :: List';
		$this->view->headTitle($this->view->Title,true);
		
		$tblProductSpecial = new Product_Model_Productspecial();
		$this->view->Items = $tblProductSpecial->listItem($this->_arrParam, array('task'=>'admin-list'));
				
		$totalItem  = $tblProductSpecial->countItem($this->_arrParam);
	...
	public function addAction(){
		...
		if($this->_request->isPost()){
			...
			if($validate->checkError() == true){
				...
				$tblProductSpecial 	= new Product_Model_Productspecial();	
				$rs				= $validate->getData();			
				$pro_id=$tblProductSpecial->saveItem($rs,array('task'=>'admin-add'));
       	...
       	$this->view->Title = 'Product :: Product Special manager :: Add';
		$this->view->headTitle($this->view->Title,true);
	...
	public function infoAction(){
		$this->view->Title = 'Product :: Product Special manager :: Information';
		$this->view->headTitle($this->view->Title,true);
		
		$tblProductSpecial = new Product_Model_Productspecial();
		$this->view->Item = $tblProductSpecial->getItem($this->_arrParam,array('task'=>'admin-info'));
	...
	public function editAction(){
		...
		if($this->_request->isPost()){
			...
			if(!$tblProductSpecial->isOwner($tblProductSpecial->getItem($this->_arrParam,array('task'=>'admin-edit')))){//kiểm tra xem có phải của người đó hay ko
                //Hien thi thong bao loi
				
                $this->view->errors=array('Bạn không có quyền chỉnh sửa nội dung');
				
            }
            elseif($validate->checkError() == true){
				//Hien thi thong bao loi
				$this->view->errors = $validate->getMessageError();
				$this->view->Item = $validate->getData();
			}else{
				//Luu vao database
				$rs			= $validate->getData();			
				$tblProductSpecial->saveItem($rs,array('task'=>'admin-edit'));
				$this->view->errors = array('success');
				$this->view->Item	= $rs;
        ...
        $this->view->Title = 'Product :: Product specical manager :: Edit';
		$this->view->headTitle($this->view->Title,true);
		...
		$this->view->listPhoto= $tblPhoto->listItem($this->_arrParam,array('task'=>'list-by-proid','type'=>2));
		        
		//get item info
        	
		$this->view->Item = $tblProductSpecial->getItem($this->_arrParam,array('task'=>'admin-edit'));
		
		
	}

	public function deleteAction(){
		$this->view->Title = 'Product :: Product Special manager :: Delete';
		$this->view->headTitle($this->view->Title,true);
		if($this->_request->isPost()){
			$tblProductSpecial = new Product_Model_Productspecial();
            if(!$tblProductSpecial->isOwner($tblProductSpecial->getItem($this->_arrParam,array('task'=>'admin-edit')))){//kiểm tra xem có phải của người đó hay ko
            ...
            {
                $tblProductSpecial->deleteItem($this->_arrParam,array('task'=>'admin-delete'));
	...
	public function statusAction(){
		$tblProductSpecial = new Product_Model_Productspecial();		
		$tblProductSpecial->changeStatus($this->_arrParam);
	...
	public function multiDeleteAction(){
		...
		if($this->_request->isPost()){
			...
			$tblProductSpecial->deleteItem($this->_arrParam,array('task'=>'admin-multi-delete'));
			$this->_redirect($this->_actionMain);
		}
		$this->_helper->viewRenderer->setNoRender();
	}
	...
	public function sortAction(){
		if($this->_request->isPost()){
			$tblProductSpecial = new Product_Model_Productspecial();
			$tblProductSpecial->sortItem($this->_arrParam);
			$this->_redirect($this->_actionMain);
		}
