        ...
        elseif($options['task'] == 'list-newest'){//liet ke danh sách các s?n ph?m c?a doanh nghi?p
            ...
            if($options['local']=='index')
            {
                ...
                foreach($result as $key=>$info)
                {
                    ...
                    $newArray[$key]['link']=$objView->baseUrl('san-pham/'.$objView->cmsRewriteLink($info,array('task'=>'product-detail')));;
                }
                $result=$newArray;
            }
        }
		elseif($options['task'] == 'list-product'){//liet ke danh sách các s?n ph?m c?a doanh nghi?p
            $arrayField=array('id','name','image','summary','price');
            $select = $db->select()
						 ->from($this->_name.' AS p',$arrayField)   
